"""Goal → room-indicator mapping for exploration navigation (Milestone M1).

Where ``landmark_map.py`` answers *"what COCO classes is this landmark?"*, this
module answers *"what objects tell me I've arrived in this room?"*. The exploration
controller (future M2+) uses it for the **arrival test**: a room is recognised as the
goal once the objects detected in it match the goal's indicator set.

Design mirrors ``landmark_map.py``:
- Hardcoded table, lowercased keys, no I/O at import.
...
Every indicator below is drawn from the COCO-80 set, so the existing YOLOv8n detector
produces them with **no model change**. (``door`` is the only out-of-COCO class in the
project and is intentionally not used as a room indicator.)
...
def evaluate_arrival(goal: str, confirmed_classes: Iterable[str]) -> dict:
    ...
    this room (temporal consistency is the caller's job — same 2-of-3 rule the rest
    of the system uses). Arrival rule: >=1 primary OR >=2 secondary indicators.
    """
