...
\begin{tikzpicture}[node distance=16mm and 34mm]
  ...
  \draw[arr] (listen) -- node[lbl, below right]{command\_recognized\\(find X)} (obj);
  \draw[arr] (listen) -- node[lbl, above right]{command\_recognized\\(navigate to Y)} (nav);
  ...
  \draw[arr] (obj.west) to[bend right=12] node[lbl, above left]{task\_abort} (listen.north);
  \draw[arr] (nav.west) to[bend left=12] node[lbl, below right]{task\_abort} (listen.south);
  \draw[arr] (ret.south) to[bend left=22] node[lbl, below left]{cleanup\_done} (idle.south);
\end{tikzpicture}}
