...
\begin{tikzpicture}[node distance=6mm]
  ...
  % WS link
  \draw[arr, <->] (client.east) -- node[lbl, above]{WebSocket /ws}
        node[lbl, below]{JSON + tagged binary\\0x01 JPEG frame $\rightarrow$\\0x02 PTT audio $\rightarrow$\\$\leftarrow$ 0x03 TTS MP3} (server.west);
\end{tikzpicture}}
\caption{System architecture: a sensing-only browser client and a single-process
...
\begin{tikzpicture}[node distance=9mm and 16mm]
  ...
  \node[state, right=42mm of listen] (ret) {Returning\\ToIdle};
...
\end{tikzpicture}}
\caption{The session FSM. Every transition is driven by an explicit event; the only
