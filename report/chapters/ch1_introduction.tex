...
The system organizes assistance into three task families:
...
\begin{itemize}
    \item \textbf{Object Allocation}: locates a requested item (``find my cup'') and
          ...
          per-class size priors.
    \item \textbf{Reach Guidance}: once the object is within arm's reach, tracks the
          user's hand and steers it to the target (``move your hand to the right
          \ldots{} almost there''), completing the task automatically when the
          fingertip reaches the object.
...
\begin{itemize}
    ...
    \item \textbf{FSM-Governed, Testable Assistive Architecture}: explicit
          state-machine control of every session, with the decision logic of all
          three task families isolated from models and I/O and verified by 286
...
\textbf{Chapter~\ref{ch:task_families}} details the three task families and their
